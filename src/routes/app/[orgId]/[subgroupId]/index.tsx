...
export const useStatus = routeLoader$(async (req) => {
  ...
  const { data, error } = await supabase.from("tickets").select("closed_at, assigned_to").eq("org_id", req.params.orgId).eq("subgroup_id", req.params.subgroupId)
  if (error) throw req.error(500, error.message)

  let assigned = 0, completed = 0, open = 0, closed = 0;
