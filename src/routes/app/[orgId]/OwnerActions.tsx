...
export const useInvite = routeAction$(
  async ({ email, message }, req) => {
    ...
    if (user.email === email)
      return req.fail(500, { message: "You cannot invite yourself" });
    ...
    if (!org || org.owner_id !== user.id)
      return req.fail(403, { message: "forbidden" });
    ...
    if (res.status !== 200)
      return req.fail(res.status, { message: await res.text() });
  },
  zod$({
    email: z
      .string()
    ...
    <section class="my-10">
      ...
      <div class="grid grid-cols-1 gap-4">
        {!showInviteForm.value ? (
          <button
            onClick$={() => (showInviteForm.value = true)}
          ...
          >
            ...
            <h3 class="my-4 text-xl font-bold">Invite a user</h3>
            <p class="my-4 text-lg">Enter the user's email to invite them</p>
            <div class="space-y-2">
            ...
            <div class="mt-2 space-y-2">
              ...
              <textarea
                ...
                name="message"
                id="message"
                placeholder="Enter a message to send to them"
              />
        ...
        <Link
          class="flex items-center gap-4 rounded border border-gray-500 bg-gray-100 px-4 py-2 shadow-sm dark:bg-gray-800"
          href={`/app/${orgId}/new-subgroup`}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
